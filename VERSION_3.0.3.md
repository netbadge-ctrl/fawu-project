...
### 🎯 新增项目排序问题
- **问题描述**: 新增项目时，由于应用了排序规则，待编辑的新项目行会被排序到其他位置，不在第一行显示
- **修复方案**: 在排序逻辑中添加优先级判断，新项目和正在编辑的项目始终排在最前面
- **技术实现**: 使用 `editingId` 和 `isNew` 标记识别特殊项目，不受任何排序规则影响
- **用户体验**: 点击“新增项目”后，编辑行始终显示在列表第一行，无需滚动页面
