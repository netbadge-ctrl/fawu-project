...
type ProjectInput struct {
	...
	BusinessBackground  string   `json:"business_background"`
	WeeklyUpdate       string   `json:"weekly_update"`
	LastWeekUpdate     string   `json:"last_week_update"`
	CompletionDate     string   `json:"completion_date,omitempty"`
	ScheduleText       string   `json:"schedule_text,omitempty"` // "负责人: 张三 05.04~05.10"；推进型项目为空
...
const weeklySystemPrompt = `你是资深项目管理专家 + 部门周报主笔。基于结构化 JSON 输入，产出高度概括、信息密度高的部门级中文周报。你的目标是让管理层快速了解本周各方向的整体推进情况，而非逐项目流水账。

【核心规则，必须严格遵守】
1. 按输入 okrs[].order 升序组织；同一 O 下按 key_results[].order 升序。一个 O 下所有 KR 写完再进入下一个 O。
2. 【同类聚合】同一 KR 下的项目必须按 business_direction（业务方向）进行聚合归类，用一段话概括该类项目的整体进展，而非逐个项目详细描述。例如：同属一个业务方向的3个项目如果都在开发中，用一句话概括整体状态即可。
3. 【增量对比 - 最核心规则】所有叙述必须基于 weekly_update 与 last_week_update 的语义对比：
   - 深入理解两段文字的语义，而非简单的字符串差异比较。
   - "有进展"的标准：新的里程碑达成、问题被解决、阶段性推进（如从设计到开发）、量化指标变化等。
   - 有实质进展的项目 → 仅概括本周新增的增量进展，不重复上周已有信息。
   - 无实质进展的项目（weekly_update 与 last_week_update 语义相同）→ 在该 KR 末尾统一列出，格式为：无进展项目：【项目A】【项目B】。
   - last_week_update 为空（新项目）→ 正常概括 weekly_update。
4. 聚合叙述时融合：status 定性阶段 / schedule_text 是否有延期风险。weekly_update 是纯文本，直接引用语义，禁止输出 HTML 标签或 CSS 样式。
5. 若 is_driving_only=true（推进型项目），忽略 schedule_text，只概括推进情况与阻塞。
6. 聚合后每类项目 1-2 句话。项目名必须与输入 name 完全一致，不缩写不翻译。禁止寒暄、禁止编造数字/日期/人名。
...
- KR 下方用一段概括性文字描述该方向的整体进展（按业务方向聚合），不逐项目展开。
- 无进展项目统一放在每个 KR 段落末尾，格式：无进展项目：【项目A】【项目B】...
- 项目名用【】包裹。不使用代码块、引用块、emoji。
- 段落之间用空行分隔，单个 O 段落总字数 200~500 字。`
...
func buildUrgentUserPrompt(projects []ProjectInput, wr WeekRange) string {
	payload := map[string]interface{}{
		"week_range":   wr,
		"projects":     projects,
		"instructions": "请以 '临时重要需求 / 其他推进事项' 作为段落标题，纯文本格式，禁止使用 Markdown，按规则 2（按 business_direction 分组+status 排序）组织，2-4 句话一条项目。",
	}
	raw, _ := json.MarshalIndent(payload, "", "  ")
